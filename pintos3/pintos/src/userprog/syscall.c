...
read(int fd, void* buffer, unsigned size){
	...
	// ************************ WANT PINNING *************************** //
	/*
	if(pagedir_get_page(thread_current()->pagedir,pg_round_down(buffer))){
		printf("read 1\n");
#ifdef VM
		if(!find_entry_by_upage(thread_current()->pagedir,pg_round_down(buffer))){
			printf("entry not found\n");
			void* ppage=palloc_get_page(PAL_USER);		
			if(!ppage){
				swap_out();
				ppage = palloc_get_page(PAL_USER);
			}
			push_invt_tb(thread_current()->pagedir,pg_round_down(buffer),ppage,false);
		}
#endif
...
write(int fd, const void*buffer, unsigned size){
	...
	/*
		...
		if(!find_entry_by_upage(thread_current()->pagedir,pg_round_down(buffer))){
			...
			push_invt_tb(thread_current()->pagedir,pg_round_down(buffer),ppage,true);
		}
#endif
...
syscall_handler (struct intr_frame *f) 
{
	...
	if(syscall_no<0 || syscall_no>15)
		thread_exit();
	//hex_dump((uintptr_t)f->esp,f->esp,30,1);
	switch(syscall_no){
		case SYS_HALT:
			halt(); break;
		case SYS_EXIT:
